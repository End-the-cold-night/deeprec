...
from deeprec.ranking.ctr import Nffm
...
def _eval(sess, model):
  ...
  if best_auc < test_gauc:
    best_auc = test_gauc
    model.save(sess, 'save_nffm_path/ckpt')
  pickle.dump(all_result, open("auc_nffm_all_result.pkl","wb"))
  return None
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
  model = Nffm(feature_config_dict)
