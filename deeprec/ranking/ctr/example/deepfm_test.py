...
from deeprec.ranking.ctr import DeepFm
...
def _eval(sess, model):
  ...
  if best_auc < test_gauc:
    best_auc = test_gauc
    model.save(sess, 'save_DeepFm_path/ckpt')
  pickle.dump(all_result, open("auc_DeepFm_all_result.pkl","wb"))
  return None
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
  model = DeepFm(feature_config_dict)
