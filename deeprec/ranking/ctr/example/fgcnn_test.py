...
from deeprec.ranking.ctr import Fgcnn
...
def _eval(sess, model):
  ...
  if best_auc < test_gauc:
    best_auc = test_gauc
    model.save(sess, 'save_Fgcnn_path/ckpt')
  pickle.dump(all_result, open("auc_Fgcnn_all_result.pkl","wb"))
  return None
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
  model = Fgcnn(feature_config_dict)
