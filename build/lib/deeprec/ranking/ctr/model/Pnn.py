...
from ..layer.common_layer import ipnn, opnn
from ..utils.utize import cal_feature_number, get_linear_embedding, get_embedding, get_sequence_embedding


class Pnn(object):

    def __init__(self, feature_config_dict, embedding_size=8, l2_reg_linear=0.00001, opnn_deep_init_size=20,
                 ipnn_deep_init_size=20, dnn_layers=[40, 20], linear_deep_init_size=20,
                 sequence_sprase_use_same_embedding=True, l2_reg_embedding=0.00001, use_inner=True,
                 init_std=0.0001, seed=1024, task='binary'):
        """
        :param feature_config_dict:
        :param embedding_size:
        :param l2_reg_linear:
        :param opnn_deep_init_size: opnn输出个数
        :param ipnn_deep_init_size: ipnn输出个数
        ...
        # ipnn
        ipnn_out = ipnn(self.sprase_data_embedding, reduce_sum=1)  # 无参数，可考虑加入参数
        ipnn_out = tf.layers.dense(ipnn_out, ipnn_deep_init_size)
        # opnn
        opnn_out = opnn(self.sprase_data_embedding, opnn_deep_init_size)
        out = tf.concat([ipnn_out, opnn_out], axis =1)
       ...
       # DNN part
        for x in dnn_layers:
            self.logits = tf.layers.dense(out, x, activation=None,
