...
from ..layer.common_layer import din
from ..utils.utize import cal_feature_number, get_linear_embedding, get_embedding, get_sequence_embedding


class Din(object):

    def __init__(self, feature_config_dict, embedding_size=8, l2_reg_linear=0.00001, target_num=-1,
                 task='binary'):
        ...
        queries = self.sprase_data_embedding[:, target_num, :]
        out = din(queries, self.sequence_data_embedding)

        self.sprase_data_embedding = tf.reshape(self.sprase_data_embedding,
                                                [-1, self.number_of_sprase_feature * embedding_size])

        out = tf.concat([out, self.sprase_data_linear_embedding, self.sprase_data_embedding], axis=1)
